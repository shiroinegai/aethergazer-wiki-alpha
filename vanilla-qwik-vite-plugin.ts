    ...
    async configResolved(resolvedConfig) {
      ...
      if (
        config.plugins.some((plugin) =>
          ['astro:build', 'vite-plugin-qwik', 'solid-start-server'].includes(
            plugin.name
          )
        )
      ) {
    ...
    async transform(code, id, ssrParam) {
      ...
      let output = await processVanillaFile({
        ...
        serializeVirtualCssPath: async (args) => {
          ...
          // Vite doesn't like exporting the string directly
          return `const __css=${JSON.stringify(source)};export default __css;`;
      ...
      output = output.replace(/^\s*export default\s.*$/m, '');
      console.log({ output });
      return {
        code: output,
        map: { mappings: '' },
